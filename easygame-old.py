...
class _Image:
    def __init__(self, img, ui=False):
        import pyglet
        self._img = img
        self._sprite = pyglet.sprite.Sprite(img, batch=None if not ui else _ctx._ui_batch)
...
def load_image(path, ui=False):
    ...
    """
    import pyglet
    return _Image(pyglet.resource.image(path, ui))
